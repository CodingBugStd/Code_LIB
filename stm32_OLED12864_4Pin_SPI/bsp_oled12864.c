...
//OLED12864 4Pin_SPI 移植测试
#if 0
void Transplant_Test(void)
{
    uint8_t TestCmd[128];
    for(uint8_t temp=0;temp<128;temp++)
        TestCmd[temp] = 0xf0;
    BSP_OLED12864_Init();
    OLED12864_Set_Position(0,0);
    OLED12864_Send_NumByte(TestCmd,128,OLED_DATA);
    OLED12864_Refresh();
}
#endif
...
void OLED12864_Set_Position(uint8_t page,uint8_t x)
{
    ...
    OLED12864_Send_Byte(((0xf0&x)>>4)|0x10,OLED_CMD);   //x坐标高四位设定
    OLED12864_Send_Byte(0x0f&x,OLED_CMD);               //x坐标低四位设定
}
